...
//				print_r(mysqli_error($GLOBALS["con1"]));
				echo "</font>";  
			}
		}
		else
...
else
{
	...
	$key = mysqli_real_escape_string($GLOBALS["con1"], $key);
	//echo $key;
	//Query table to verify your result
	$sql="SELECT 1 FROM $table WHERE $col1= '$key'";
	//echo "$sql";
	$result=mysqli_query($GLOBALS["con1"], $sql)or die("error in submittion of Key Solution".mysqli_error($GLOBALS["con1"]));
