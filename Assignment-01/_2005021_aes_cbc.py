...
# For CBC mode padding is required
PADDING=True
...
def inv_shift_rows(plain_tex_blockt: list[list[BitVector]]):
    # Inverse shift rows of the block
    for i in range(1, WORD_PER_KEY):
        plain_tex_blockt[i] = rotate_word(plain_tex_blockt[i], i, RIGHT)
    return plain_tex_blockt

# Function to Inverse Mix columns
def inv_mix_columns(plain_tex_blockt: list[list[BitVector]]):
    result = []
    # Inverse mix columns of the block
    for i in range(WORD_PER_KEY):
        temp = [BitVector(size=8) for _ in range(WORD_PER_KEY)]
        for j in range(WORD_SIZE):
            for k in range(WORD_SIZE):
                temp[j] ^= InvMixer[i][k].gf_multiply_modular(plain_tex_blockt[k][j], AES_modulus, 8)
        result.append(temp)
    return result

# Function to perform the Inverse SubBytes step
def inv_sub_bytes(plain_tex_blockt: list[list[BitVector]]):
    # Inverse substitute each byte in the block using InvSbox
    for i in range(WORD_PER_KEY):
        for j in range(WORD_SIZE):
            plain_tex_blockt[i][j] = BitVector(intVal=InvSbox[plain_tex_blockt[i][j].intValue()], size=8)
    return plain_tex_blockt

# Function to decrypt a 128 bit block
def decrypt_block(block_1d: list[BitVector], round_keys: list[list[list[BitVector]]]) -> list[BitVector]:
    # Convert the block to a 4x4 matrix
    block_2d = convert_text_to_block(block_1d)
    ...
    for i in range(round_count - 2, 0, -1):
        block_2d = inv_shift_rows(block_2d)
        block_2d = inv_sub_bytes(block_2d)
        block_2d = add_round_key(block_2d, round_keys[i])
        block_2d = inv_mix_columns(block_2d)
    # Perform the last round without MixColumns
    block_2d = inv_shift_rows(block_2d)
    block_2d = inv_sub_bytes(block_2d)
    block_2d = add_round_key(block_2d, round_keys[0])
...
# Utility function for encrypting
def encrypt(key_bitvector: list[BitVector], text_bitvector: list[BitVector], IV: list[BitVector]):
    ...
    for i in range(0, len(text_bitvector), BLOCK_SIZE):
        ...
        IV = encrypted_block
        cipher_text_bitvectors.append(encrypted_block)

    # Convert the list of list of BitVectors to a single list of BitVectors
    cipher_text_bitvector = [bit for block in cipher_text_bitvectors for bit in block]
    
    return cipher_text_bitvector

# Utility function for decrypting
def decrypt(key_bitvector: list[BitVector], text_bitvector: list[BitVector], IV: list[BitVector]) -> list[BitVector]:
    # Determine the round keys
    start_time = time.time()
    round_keys = key_schedule(key_bitvector)
    end_time = time.time()
    ...
    # Divide the plaintext into 16 byte blocks and prepare args for each block
    plain_text_bitvectors = []
    for i in range(0, len(text_bitvector), BLOCK_SIZE):
        block = text_bitvector[i : min(i + BLOCK_SIZE, len(text_bitvector))]
        decrypted_block = decrypt_block(block, round_keys)
        decrypted_block = XOR(decrypted_block, IV)
        IV = block
        plain_text_bitvectors.append(decrypted_block)

    # Convert the list of list of BitVectors to a single list of BitVectors
    plain_text_bitvector = [bit for block in plain_text_bitvectors for bit in block]
...
def encrypt_plain_text(key: str, plain_text: str) -> str:
    ...
    # Randomly generate a IV
    IV = [BitVector(intVal=random.randint(0, 255), size=8) for _ in range(BLOCK_SIZE)]

    cipher_text_bitvector = IV + encrypt(key_bitvector, plain_text_bitvector, IV)
...
def decrypt_ciphered_text(key: str, cipher_text: str) -> str:
    ...
    key_bitvector = convet_string_to_bitvector(key)
    IV_cipher_text_bitvector = convet_string_to_bitvector(cipher_text)

    # Extract the IV and ciphertext from the ciphertext
    IV = IV_cipher_text_bitvector[:BLOCK_SIZE]
    cipher_text_bitvector = IV_cipher_text_bitvector[BLOCK_SIZE:]

    # Decrypt the ciphertext using the round keys
    deciphered_text_bitvector = decrypt(key_bitvector, cipher_text_bitvector, IV)
