...
def check_key_validity(key: str) -> None:
...
def convet_string_to_bitvector(string: str) -> list[BitVector]:
    return [BitVector(textstring=char) for char in string]

# Function to convert a bit vector to a string
def convet_bitvector_to_string(bitvector: list[BitVector]) -> str:
    text = ""
    for b in bitvector:
        text += b.get_bitvector_in_ascii()
    return text

# Function to convert bytes to a bit vector
def convet_bytes_to_bitvector(bytes: bytes) -> BitVector:
    # Convert each byte to its ASCII representation
    return [BitVector(intVal= byte, size=8) for byte in bytes]

# Function to convet a bit vector to bytes
def convet_bitvector_to_bytes(bitvector: list[BitVector]) -> bytes:
    # Convert each byte to its ASCII representation
    return bytes([b.intValue() for b in bitvector])

# Function to convert a bit vector to a string
def print_bitvector_as_ascii(key: list[BitVector]) -> None:
...
def print_bitvector_as_hex(key: list[BitVector]) -> None:
    for bitvector in key:
...
def get_round_counts(key_length: int) -> int:
...
def generate_round_constants(count: int) -> list[list[BitVector]]:
...
def rotate_word(word: list[BitVector], n: int, direction: int) -> list[BitVector]:
...
def substitute_word(word: list[BitVector]) -> list[BitVector]:
...
def XOR(a: list[BitVector], b: list[BitVector]) -> list[BitVector]:
    # XOR two lists of bytes
    return [a[i] ^ b[i] for i in range(min(len(a), len(b)))]

# Key schedule function
def key_schedule(key: list[BitVector]) -> list[list[list[BitVector]]]:
    # determine the number of rounds based on key length
...
def sub_bytes(plain_tex_blockt: list[list[BitVector]]) -> list[list[BitVector]]:
...
def shift_rows(plain_tex_blockt: list[list[BitVector]]) -> list[list[BitVector]]:
...
def mix_columns(plain_tex_blockt: list[list[BitVector]]) -> list[list[BitVector]]:
    result = []
...
def add_round_key(plain_tex_block: list[list[BitVector]], round_key: list[list[BitVector]]) -> list[list[BitVector]]:
    # XOR the plaintext with the round key
    for i in range(WORD_PER_KEY):
        for j in range(WORD_SIZE):
            plain_tex_block[i][j] = plain_tex_block[i][j] ^ round_key[i][j]
    # print_block(plain_tex_blockt)
    return plain_tex_block
...
def encrypt_decrypt(key_bitvector: list[BitVector], text_bitvector: list[BitVector], nonce: list[BitVector]) -> list[BitVector]:
...
# Function to encrypt the BitVector
def encrypt_bytes(key_bytes: bytes, plain_text_bytes: bytes) -> bytes:
    # Convert bytes to BitVectors
    key_bitvector = convet_bytes_to_bitvector(key_bytes)
    plain_text_bitvector = convet_bytes_to_bitvector(plain_text_bytes)
    ...
    # Convert the list of BitVectors to bytes
    return convet_bitvector_to_bytes(cipher_text_bitvector)

def decrypt_bytes(key_bytes: bytes, cipher_text_bytes: bytes) -> bytes:
    # Convert bytes to BitVectors
    key_bitvector = convet_bytes_to_bitvector(key_bytes)
    cipher_text_bitvector = convet_bytes_to_bitvector(cipher_text_bytes)

    # Extract the nonce and ciphertext from the ciphertext
    nonce = cipher_text_bitvector[:BLOCK_SIZE]
    cipher_text_bitvector = cipher_text_bitvector[BLOCK_SIZE:]
    ...
    # Convert the list of BitVectors to bytes
    return convet_bitvector_to_bytes(deciphered_text_bitvector)

# Function to encrypt the plaintext using the round keys
def encrypt_plain_text(key: str, plain_text: str) -> str:
    # preprocess the key and plaintext to Bit vectors
    ...
    return convet_bitvector_to_string(cipher_text_bitvector)
...
def decrypt_ciphered_text(key: str, cipher_text: str) -> str:
    ...
    # Convert the list of BitVectors to string
    return convet_bitvector_to_string(deciphered_text_bitvector)

# Function to encrypt file chunk by chunk
def encrypt_file_in_chunks(key: str, file_path: str, chunk_size: int):
    key_bitvecotr = convet_string_to_bitvector(key)
    nonce = [BitVector(intVal=random.randint(0, 255), size=8) for _ in range(BLOCK_SIZE)]
    read_block_size  = int(127 * BLOCK_SIZE)
    byte_encrypted = 0
    with open(file_path, 'rb') as file:
        while True:
            ...
            if not block:
                break
            # Encrypt the chunk
            block_bitvector = convet_bytes_to_bitvector(block)

            if(read_block_size == 127 * BLOCK_SIZE):
                encrypted_block = nonce + encrypt_decrypt(key_bitvecotr, block_bitvector, nonce)
                read_block_size += BLOCK_SIZE
            else:
                encrypted_block = encrypt_decrypt(key_bitvecotr, block_bitvector, nonce)

            # Send the encrypted block chunk by chunk
            for i in range(0, len(encrypted_block), chunk_size):
                chunk = encrypted_block[i:min(i + chunk_size, len(encrypted_block))]
                yield convet_bitvector_to_bytes(chunk)

            byte_encrypted += len(block)
            print("Currently encrypted ", byte_encrypted, "bytes and total bytes are ", byte_encrypted)

# Function to decrypt file chunk by chunk
def decrypt_file_in_chunks(key: str, file_path: str, chunk_iterator) -> None:
    key_bitvecotr = convet_string_to_bitvector(key)
    write_block_size = int(127 * BLOCK_SIZE)
    total_bytes = 0
    with open(file_path, 'wb') as file:
        for chunk in chunk_iterator:
            if write_block_size == 127 * BLOCK_SIZE:
                chunk_bitvector = convet_bytes_to_bitvector(chunk)
                # Extract the nonce from the first block
                nonce = chunk_bitvector[:BLOCK_SIZE]
                ciphered_text_bitvector = chunk_bitvector[BLOCK_SIZE:]
                # Decrypt the chunk
                decrypted_block_bitvector = encrypt_decrypt(key_bitvecotr, ciphered_text_bitvector, nonce)
                write_block_size += BLOCK_SIZE
            else:
                # Decrypt the chunk
                ciphered_text_bitvector = convet_bytes_to_bitvector(chunk)
                decrypted_block_bitvector = encrypt_decrypt(key_bitvecotr, ciphered_text_bitvector, nonce)
            # Write the decrypted block to the file
            deciphered_block = convet_bitvector_to_bytes(decrypted_block_bitvector)
            file.write(deciphered_block)
            total_bytes += len(deciphered_block)
            print("Currently decrypted ", total_bytes, "bytes and total bytes are ", total_bytes)
